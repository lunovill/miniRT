...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/02/25 03:50:23 by lunovill          #+#    #+#             */
/*   Updated: 2023/02/25 03:50:24 by lunovill         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	rt_cylinder(t_miniRT *data, t_cylinder *cy, t_tpl4f point)
{
	...
	while (data->l[++i])
	{
		...
		{
			diffuse = rt_diffuse(cy->color, cy->normal, r.vector, data->l[i]);
			if (diffuse.x)
				color += diffuse + rt_specular(
						cy->normal, r.vector, data->c->view.s89ae, data->l[i]);
		}
	}
